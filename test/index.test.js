...
describe('Handling Events with JavaScript', () => {
  ...
  test('should display the key pressed by the user', async () => {
    const event = new document.defaultView.KeyboardEvent('keydown', { key: 'A' })
    document.dispatchEvent(event)

    // Need to add a wait to get the test to capture the input
    await new Promise(r => setTimeout(r, 0))
    expect(keyPressDisplay.textContent).toContain('A')
  })

  test('should display user input in real-time', async () => {
    textInput.value = 'Hello'
    const event = new document.defaultView.Event('input')
    textInput.dispatchEvent(event)

    // Need to add a wait to catch this for the test
    await new Promise(r => setTimeout(r, 0))
